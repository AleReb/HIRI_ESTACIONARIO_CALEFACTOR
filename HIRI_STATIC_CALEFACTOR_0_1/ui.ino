...
extern bool heaterConnected;
extern bool heaterVersionKnown;
extern String heaterFirmwareVersion;
extern String hdopStr, gpsAlt;
extern uint32_t sendCounter, sdSaveCounter;
extern float heaterExternalTemp, heaterShtTemp, heaterShtHum;
extern float heaterBmeTemp, heaterBmeHum, heaterThermoTemp;
extern bool heaterRelayOn, heaterByHumidity, heaterByTemperature, heaterEmergencyMode;
extern uint16_t PM10, PM1;
extern bool sendCurrentMeasurement();
extern bool saveMeasurementToSD(const String &values,
                                const String &connectionStatus);
extern String lastHttpMeasurementValues;
extern bool heaterRequestTimedTest(const String &secondsText);
...
// Menú Principal
const char *topItems[] = {"PM2.5", "CALEFACTOR", "TEMPERATURA", "HUMEDAD",
                          "EMPEZAR MUESTREO", "OPCIONES"};
...
// Menú de “Información”
// Menú de “Información”
const char *infoItems[] = {"VERSION", "REDES", "GPS", "GUARDADO", "WIFI SD (ON/OFF)", "VOLVER"};
...
void drawSensorValue(uint8_t idx) {
  ...
  if (idx == 0) { // PM2.5
    ...
    baseF = "PM 2.5 (ug/m3)";
  } else if (idx == 1) { // Calefactor
    u8g2.setFont(u8g2_font_5x7_tf);
    String heaterTitle = heaterVersionKnown
                              ? String("CALEFACTOR FW ") + heaterFirmwareVersion
                              : (heaterConnected ? "CALEFACTOR OK"
                                                 : "CALEFACTOR SIN DATOS");
    u8g2.drawStr(3, 22, heaterTitle.c_str());
    snprintf(buf, sizeof(buf), "SHT %4.1fC %4.1f%%", heaterShtTemp, heaterShtHum);
    u8g2.drawStr(3, 34, buf);
    snprintf(buf, sizeof(buf), "TERM %4.1fC R:%d", heaterThermoTemp, heaterRelayOn ? 1 : 0);
    u8g2.drawStr(3, 44, buf);
    snprintf(buf, sizeof(buf), "BME %4.1fC %4.1f%%", heaterBmeTemp, heaterBmeHum);
    u8g2.drawStr(3, 54, buf);
    return;
  } else if (idx == 2) { // Temp
    if (isnan(pmsTempC))
...
// Vista principal de estación con rotación de pantallas.
void drawStationView() {
  ...
  } else if (screenIdx == 2) { // --- SCREEN 2: HEATER ---
    String heaterTitle = heaterVersionKnown
                              ? String("CALEFACTOR FW ") + heaterFirmwareVersion
                              : "CALEFACTOR";
    u8g2.drawStr(0, 16, heaterTitle.c_str());
    int y = 26;
    u8g2.setCursor(0, y);
    if (heaterConnected) {
      u8g2.print("SHT: " + String(heaterShtTemp, 1) + "C / " + String(heaterShtHum, 1) + "%");
    } else {
      u8g2.print("CALEFACTOR: SIN DATOS");
    }
    y += 8;
    u8g2.setCursor(0, y);
    u8g2.print("BME: " + String(heaterBmeTemp, 1) + "C / " + String(heaterBmeHum, 1) + "%");
    y += 8;
    u8g2.setCursor(0, y);
    u8g2.print("TERM: " + String(heaterThermoTemp, 1) + "C REL:" + String(heaterRelayOn ? "ON" : "OFF"));
    y += 8;
    u8g2.setCursor(0, y);
    u8g2.print("H:" + String(heaterByHumidity) + " T:" + String(heaterByTemperature) + " E:" + String(heaterEmergencyMode));

  } else if (screenIdx == 3) { // --- SCREEN 3: SYSTEM ---
    u8g2.drawStr(0, 16, "ESTADO ESTACION");
    u8g2.setCursor(0, 26);
...
void renderDisplay() {
  ...
  uint8_t screenCount = config.gnssEnabled ? 4 : 3;
  if (millis() - lastDebugRotationMs >= DEBUG_ROTATION_INTERVAL_MS) {
    lastDebugRotationMs = millis();
    debugScreenIndex = (debugScreenIndex + 1) % screenCount;
  }
  drawStationView();
  u8g2.sendBuffer();
}
...
void ui_btn1_click() {
  ...
  {
    // BTN1 prueba HTTP, SD y finalmente solicita 10 s de calefacción.
    bool txOk = sendCurrentMeasurement();
    hasHttpAttempted = true;
    lastHttpActivityMs = millis();
    lastHttpOk = txOk;

    bool sdSaved = saveMeasurementToSD(
        lastHttpMeasurementValues, txOk ? "HTTP_MANUAL_OK" : "HTTP_MANUAL_FALLO");
    lastTxPayloadSdOk = sdSaved;
    lastSdOk = sdSaved;
    lastSdActivityMs = millis();

    const uint16_t heaterTestSeconds = 10;
    bool heaterCommandSent =
        heaterRequestTimedTest(String(heaterTestSeconds));

    char resultMsg[21];
    snprintf(resultMsg, sizeof(resultMsg), "TX:%s SD:%s CAL:%s",
             txOk ? "OK" : "F", sdSaved ? "OK" : "F",
             heaterCommandSent ? "CMD" : "F");
    showMessageFor(resultMsg, 3000);
    Serial.println("[UI] BTN1 manual action: TX " + String(txOk ? "OK" : "FAIL") +
                   " SD " + String(sdSaved ? "OK" : "FAIL") +
                   " CAL " + String(heaterCommandSent ? "CMD 10 s" : "FAIL"));
    renderDisplay();
    return;
  }
...
void ui_btn2_click() {
  ...
  if (menuDepth == 0) {
    // Main Menu
    if (menuIndex == 4) { // Empezar Muestreo (PROMPT)
