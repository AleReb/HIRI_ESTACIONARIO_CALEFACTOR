...
void processSerialCommand() {
  ...
  if (cmd == "help" || cmd == "?") {
    ...
    Serial.println(F("  cal <1-300> - Prueba el calefactor por N segundos (corte a 70 C)"));
    Serial.println(F("                 Ejemplo: cal 120"));
    ...
    Serial.println(F("\n[UI / OLED Flow]"));
    Serial.println(F("  Vista: Plantower / calefactor / sistema / GPS opcional"));
    Serial.println(F("  BTN1: prueba HTTP + SD + calefactor por 10 s"));
    Serial.println(F("  BTN2: siguiente vista (deshabilitado si BUTTON_PIN_2=-1)"));
  ...
  // -------------------- HEATER COMMANDS --------------------
    ...
    String secondsText = cmd.substring(4);
    if (!heaterRequestTimedTest(secondsText)) {
      Serial.println(F("[CALEFACTOR] Uso: cal <1-300>. Ejemplo: cal 120"));
    }
  }
