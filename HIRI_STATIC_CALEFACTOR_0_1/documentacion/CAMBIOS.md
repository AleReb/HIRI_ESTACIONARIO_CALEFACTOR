...
## [CAL V0.0.1] - 2026-07-30
### Integracion del calefactor, documentacion y PCB

- Durante la revision de arranque se envia `HIRI,1,VERSION` sin esperar ni
  bloquear el resto de la inicializacion.
- La consulta de version es opcional: un calefactor antiguo que no implemente
  `VERSION` sigue siendo detectado y utilizado mediante `HELLO` y `DATA`.
- La version recibida aparece en la fila `CALEFACTOR` de la ventana de arranque
  y en la ventana operativa del calefactor.
- Mientras no se conozca la version, HIRI repite la consulta cada 30 segundos
  durante un maximo de 3 minutos. Las consultas son no bloqueantes y luego se
  detienen; esto no afecta el uso de calefactores antiguos.
- La version incluida en `HELLO` se conserva como respaldo de compatibilidad.
- HIRI acepta el identificador vigente `CALEF` y el historico `CAL`.
- Se sincronizan README, manual tecnico, manual de usuario y descripcion de UI
  con el codigo ejecutable actual.
- Se documentan los diseños EAGLE de `PCB/` y sus validaciones pendientes.
- Se incorpora atribucion a Alejandro Rebolledo, licencia CC BY-NC 4.0 y
  descargo de responsabilidad en la raiz del proyecto.

> Las entradas siguientes describen variantes historicas y se conservan como
> registro; no representan necesariamente el firmware vigente.
